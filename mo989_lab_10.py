...
Original file is located at
    https://colab.research.google.com/drive/1un2YHFsDS9mH56EAp8eujT5N0j8UcAu1
...
running_total_strikes = (
    ...
    .with_columns(
        ...
        .cum_sum()-pl.col("daily_strikes")
        .over(pl.col("date"))
        .alias("strike_cumilative")
    )
...
damage_state = (
    strikes
    .group_by("Origin State").agg(pl.col("Cost: Total $").sum().alias("damage"))
...
type_species = (
    ...
    .with_columns(pl.col("damage").rank("dense", descending=True).over("Aircraft: Type").alias("ranking"))
...
greatest_strike_increase = (
    strikes
    .with_columns(pl.col("Collision Date and Time").str.strptime(pl.Datetime, "%m/%d/%Y %H:%M", strict=False).cast(pl.Date).alias("date"))
...
greatest_strike_increase_type = (
    strikes
    .with_columns(pl.col("Collision Date and Time").str.strptime(pl.Datetime, "%m/%d/%Y %H:%M", strict=False).cast(pl.Date).alias("date"))
    .group_by(["Aircraft: Type", "date"]).agg(pl.col("Number of Strikes").sum().alias("daily_strikes"))
    .sort(["Aircraft: Type", "date"])
    .with_columns(pl.col("daily_strikes").shift(1).over("Aircraft: Type").alias("previous_day"))
    .with_columns((pl.col("daily_strikes") - pl.col("previous_day")).alias("delta_strikes"))
    .with_columns(pl.col("delta_strikes").rank(descending=True).over("Aircraft: Type").alias("ranking"))
